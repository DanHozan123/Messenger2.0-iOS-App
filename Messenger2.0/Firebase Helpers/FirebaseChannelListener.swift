...
class FirebaseChannelListener {
    ...
    func downloadUserChannelsFromFirebase(completion: @escaping (_ allChannels: [Channel]) ->Void) {
        ...
        channelListener = FirebaseReference(collectionReferance: .Channel).whereField(kADMINID, isEqualTo: User.currentId).addSnapshotListener({ (querySnapshot, error) in
    ...
    func downloadSubscribedChannels(completion: @escaping (_ allChannels: [Channel]) ->Void) {
        ...
        channelListener = FirebaseReference(collectionReferance: .Channel).whereField(kMEMBERIDS, arrayContains: User.currentId).addSnapshotListener({ (querySnapshot, error) in
    ...
    func downloadAllChannels(completion: @escaping (_ allChannels: [Channel]) ->Void) {
        FirebaseReference(collectionReferance: .Channel).getDocuments { (querySnapshot, error) in
    ...
    func saveCannel(_ channel: Channel) {
        do {
            try FirebaseReference(collectionReferance: .Channel).document(channel.id).setData(from: channel)
    ...
    func deleteChannel(_ channel: Channel) {
        FirebaseReference(collectionReferance: .Channel).document(channel.id).delete()
    }
